  ...
  return (
    <AnimatedCardGrid className="dashboard-grid">
      ...
      <article className="panel panel-full featured-work-panel card">
        <p className="section-kicker">{data?.featuredWork?.kicker ?? '精选作品'}</p>
        <p className="featured-title">{data?.featuredWork?.title ?? '作品标题'}</p>
        <p className="featured-copy">{data?.featuredWork?.copy ?? '作品描述'}</p>
        <Link className="featured-link" href="/text-lab">
          <span className="featured-link-label">{data?.featuredWork?.linkLabel ?? '了解更多'}</span>
      ...
      <article className="panel panel-full identity-panel card">
        <div className="identity-item">
          ...
          <p className="identity-value identity-quote">{data?.identity?.motto ?? '我的座右铭'}</p>
        </div>
        <div className="identity-item">
          <p className="section-kicker">正在学习</p>
          <p className="identity-value">{data?.identity?.learning ?? '正在学习...'}</p>
        </div>
      </article>
    </AnimatedCardGrid>
  );
